...
<div class="mb-4 flex justify-between items-center">
    ...
    <a href="{{ route('coach.media.create') }}" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">
        + Yeni Medya Yükle
    </a>
</div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="p-3">
            ...
            <div class="flex space-x-1">
                <a href="{{ route('coach.media.show', $item->id) }}" class="flex-1 bg-indigo-600 text-white text-center px-2 py-1.5 rounded hover:bg-indigo-700 text-xs">
                    Görüntüle
                </a>
                @if($item->uploaded_by === Auth::id())
                <form action="{{ route('coach.media.destroy', $item->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Bu medyayı silmek istediğinize emin misiniz?');">
                    @csrf
                    @method('DELETE')
