    ...
    <div class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white {{ $isT2 ? 'rounded-2xl shadow-sm border border-slate-200' : 'rounded-lg shadow-sm border border-gray-200' }}">
            <div class="px-8 py-6 border-b {{ $isT2 ? 'border-slate-200' : 'border-gray-200' }}">
                <h2 class="text-xl font-semibold {{ $isT2 ? 'text-slate-800' : 'text-gray-900' }}">Giriş Yap</h2>
            </div>
            
            <form id="loginForm" class="p-8 space-y-5" action="{{ route('login') }}" method="POST">
                @csrf
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="block text-sm font-medium {{ $isT2 ? 'text-slate-700' : 'text-gray-700' }}">
                            Şifre
                        </label>
                        <a href="{{ route('password.request') }}" class="text-sm {{ $isT2 ? 'text-emerald-600 hover:text-emerald-700' : 'text-indigo-600 hover:text-indigo-500' }}">
                            Şifremi unuttum
                        </a>
                    </div>
                ...
                <div class="flex items-center">
                    <input id="remember" name="remember" type="checkbox" 
                           class="h-4 w-4 {{ $isT2 ? 'text-emerald-600 focus:ring-emerald-500' : 'text-indigo-600 focus:ring-indigo-500' }} border-gray-300 rounded">
                    <label for="remember" class="ml-2 block text-sm {{ $isT2 ? 'text-slate-700' : 'text-gray-700' }}">
                        Beni hatırla
                    </label>
                </div>
                ...
                <div class="text-center pt-4 border-t {{ $isT2 ? 'border-slate-200' : 'border-gray-200' }}">
                    <p class="text-sm {{ $isT2 ? 'text-slate-600' : 'text-gray-600' }}">
                        Hesabınız yok mu?
                        <a href="{{ route('register') }}" class="font-medium {{ $isT2 ? 'text-emerald-600 hover:text-emerald-700' : 'text-indigo-600 hover:text-indigo-500' }}">
                            Demo talep et
                        </a>
                    </p>
                </div>
