...
@section('page-title', 'Yeni Banka Hesabı Ekle')
@section('page-description', 'Banka hesabı ekleyin; Iyzico üye alt işyeri otomatik oluşturulur')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.bank-accounts.store') }}" method="POST" id="bankAccountForm">
            @csrf
            ...
            <div class="mb-8 border-t pt-6">
                ...
                <!-- Bilgilendirme: Otomatik üye alt işyeri -->
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                <strong>Otomatik üye alt işyeri:</strong> Bu formu kaydettiğinizde banka hesabı eklenir ve Iyzico üye alt işyeri (sub-merchant) otomatik oluşturulur. Ödemeler Iyzico üzerinden bu hesaba yönlendirilir.
                                Oluşturma başarısız olursa banka hesabı yine kaydedilir; listeden "Sub-Merchant Oluştur" butonu ile tekrar deneyebilirsiniz.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="gsm_number" id="gsm_number" value="{{ old('gsm_number', Auth::user()->school->phone ?? '') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" 
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', Auth::user()->school->email ?? '') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" 
                ...
                <div class="mb-4">
                    ...
                    <textarea name="address" id="address" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" 
                              placeholder="Tam adres bilgisi (en az 10 karakter)" required>{{ old('address', Auth::user()->school->address ?? '') }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <select name="sub_merchant_type" id="sub_merchant_type" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('sub_merchant_type') ? 'border-red-500' : '' }}" required>
                        <option value="">Seçiniz...</option>
                        <option value="PERSONAL" {{ old('sub_merchant_type') == 'PERSONAL' ? 'selected' : '' }}>Bireysel</option>
                        <option value="PRIVATE_COMPANY" {{ old('sub_merchant_type') == 'PRIVATE_COMPANY' ? 'selected' : '' }}>Şahıs Şirketi</option>
                        <option value="LIMITED_OR_JOINT_STOCK_COMPANY" {{ old('sub_merchant_type') == 'LIMITED_OR_JOINT_STOCK_COMPANY' ? 'selected' : '' }}>Limited / Anonim Şirketi</option>
                    </select>
                    @error('sub_merchant_type')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                ...
                <div id="personal_fields" class="hidden space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_surname" id="contact_surname" value="{{ old('contact_surname') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                               placeholder="Yılmaz">
                    </div>
                </div>
                ...
                <div id="private_company_fields" class="hidden space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="tax_office" id="tax_office_private" value="{{ old('tax_office') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('tax_office') ? 'border-red-500' : '' }}" 
                               placeholder="Örn: İstanbul Vergi Dairesi">
                        @error('tax_office')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Yasal Şirket Unvanı <span class="text-red-500">*</span></label>
                        <input type="text" name="legal_company_title" id="legal_company_title_private" value="{{ old('legal_company_title') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('legal_company_title') ? 'border-red-500' : '' }}" 
                               placeholder="Örn: Ahmet Yılmaz Spor Okulu">
                        @error('legal_company_title')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Vergi Numarası <span class="text-red-500">*</span></label>
                        <input type="text" name="tax_number" id="tax_number_private" value="{{ old('tax_number') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('tax_number') ? 'border-red-500' : '' }}"
                               placeholder="1234567890" required>
                        @error('tax_number')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                ...
                <div id="limited_company_fields" class="hidden space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Vergi Dairesi <span class="text-red-500">*</span></label>
                        <input type="text" name="tax_office" id="tax_office_limited" value="{{ old('tax_office') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('tax_office') ? 'border-red-500' : '' }}" 
                               placeholder="Örn: İstanbul Vergi Dairesi">
                        @error('tax_office')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Vergi Numarası <span class="text-red-500">*</span></label>
                        <input type="text" name="tax_number" id="tax_number_limited" value="{{ old('tax_number') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('tax_number') ? 'border-red-500' : '' }}"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Yasal Şirket Unvanı <span class="text-red-500">*</span></label>
                        <input type="text" name="legal_company_title" id="legal_company_title_limited" value="{{ old('legal_company_title') }}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 {{ $errors->has('legal_company_title') ? 'border-red-500' : '' }}" 
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            ...
                            Yeni hesap eklerken mevcut aktif hesabı pasif yapmanız gerekebilir.
                        </p>
                    </div>
                </div>
            </div>
