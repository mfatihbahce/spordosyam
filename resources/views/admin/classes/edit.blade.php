...
@section('page-title', 'Sınıf Düzenle')
@section('page-description', 'Sınıf bilgilerini güncelleyin')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    <div class="bg-gradient-to-r from-indigo-50 to-purple-50 px-5 py-3 border-b border-gray-200">
        <h3 class="text-base font-semibold text-gray-900 flex items-center">
            <i class="fas fa-edit text-indigo-600 mr-2 text-sm"></i>
            Sınıf Bilgileri
        </h3>
    </div>

    <form action="{{ route('admin.classes.update', $class) }}" method="POST" id="classForm" class="p-5">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" required
                           ...
                           placeholder="Örn: Futbol A Takımı" value="{{ old('name', $class->name) }}">
                    @error('name')
                ...
                <div>
                    ...
                    <select name="sport_branch_id" id="sport_branch_id" required
                        ...
                        @foreach($sportBranches as $sportBranch)
                            <option value="{{ $sportBranch->id }}" {{ old('sport_branch_id', $class->sport_branch_id) == $sportBranch->id ? 'selected' : '' }}>
                                {{ $sportBranch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="branch_id" id="branch_id"
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ old('branch_id', $class->branch_id) == $branch->id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="capacity" id="capacity" min="1"
                               ...
                               value="{{ old('capacity', $class->capacity) }}" placeholder="20">
                ...
                <div>
                    ...
                    <input type="number" name="default_credits" id="default_credits" min="1"
                           ...
                           value="{{ old('default_credits', $class->default_credits ?? 8) }}" placeholder="8">
                    <p class="mt-1 text-xs text-gray-500">Bu sınıfa kayıt olan her öğrenci için tanımlanacak ders hakkı</p>
                ...
                <div>
                    <label class="flex items-center p-2.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    @php
                        ...
                        $oldDays = old('class_days', $class->class_days ?? []);
                        $oldSchedule = old('class_schedule', $class->class_schedule ?? []);
                    @endphp
                                ...
                                <div class="flex items-center gap-1.5">
                                    <div class="flex-1">
                                        ...
                                        <input type="time" 
                                               ...
                                               value="{{ $oldSchedule[$key]['start_time'] ?? (is_string($oldSchedule[$key] ?? null) ? $oldSchedule[$key] : '') }}"
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <textarea name="description" id="description" rows="3"
                          ...
                          placeholder="Sınıf hakkında ek bilgiler, notlar veya açıklamalar...">{{ old('description', $class->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600 flex items-center">
                        <i class="fas fa-exclamation-circle mr-1 text-xs"></i>
                        {{ $message }}
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <p class="mt-1 text-xs text-gray-500">
                    ...
                    @if($class->end_date && $class->end_date < now())
                        <span class="text-red-600 font-medium">(Bitiş tarihi geçmiş - Sınıf kapalı)</span>
                    @elseif($class->end_date)
                        <span class="text-orange-600 font-medium">({{ $class->end_date->format('d.m.Y') }} tarihinde kapanacak)</span>
                    @endif
                </p>
                @error('end_date')
                    <p class="mt-1 text-xs text-red-600 flex items-center">
